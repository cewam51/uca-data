...
Le site recherche actuellement dans data.gouv.fr, data.europa.eu, Recherche Data Gouv et le catalogue officiel Melodi de l’Insee. Le même champ accepte aussi le lien d’une fiche de ces catalogues ou un lien public direct vers une table CSV, TSV ou XLSX, afin d’ouvrir immédiatement une source déjà connue. Il affiche les résultats qui contiennent une table publique exploitable ou vérifiable, choisit automatiquement la meilleure ressource disponible et l’ajoute au projet sans demander de fichier à l’utilisateur. Un classeur Excel est conservé dans son format original ; la feuille la plus tabulaire est détectée automatiquement et convertie en table interne pour l’analyse, avec le nom de la feuille conservé dans la provenance. Pour l’Insee, le CSV français est extrait automatiquement de l’archive officielle, quelle que soit sa taille ; le téléchargement et l’analyse utilisent le disque plutôt qu’un chargement complet en mémoire. Les résultats non tabulaires ou restreints ne sont pas affichés. Chaque source retenue est identifiée par l’empreinte SHA-256 de son fichier original.
